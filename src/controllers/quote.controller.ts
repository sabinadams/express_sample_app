...
export const deleteQuote: RequestHandler<DeleteQuoteSchema> = async ( req: Request<DeleteQuoteSchema>, res: Response, next: NextFunction ) => {
  const { id } = req.params
  ...
  if ( !(quote.userId === req.session.userId) ) {
    return next(new AppError('unauthorized', 'You are not authorized to delete this quote.'))
  }
